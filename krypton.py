...
# Add Planet 2 (inner habitable zone) - WITH MOON
sim.add(
    m=(1.2*u.Mearth).to(u.Msun).value,  # Mass: 1.2 Earth masses (increased for stability)
    a=0.7,                               # Semi-major axis: 0.7 AU
    e=0.04,                              # Eccentricity: 0.04 (more circular)
    omega=np.random.uniform(0, 2*np.pi) # Random argument of periapsis
)

# Add Planet 3 (middle planet) - WITH MOON
sim.add(
    m=(4*u.Mearth).to(u.Msun).value,    # Mass: 4 Earth masses (increased)
    a=1.3,                               # Semi-major axis: 1.3 AU
    e=0.05,                              # Eccentricity: 0.05 (reduced)
    omega=3.92,                          
    inc=(6*u.deg).to(u.rad).value,      # Inclination: 6 degrees (reduced)
    Omega=1.58                           
)

# Add Planet 4 (middle-outer) - WITH TWO MOONS
sim.add(
    m=(6.5*u.Mearth).to(u.Msun).value,  # Mass: 6.5 Earth masses (increased)
    a=2.1,                               # Semi-major axis: 2.1 AU
    e=0.08,                              # Eccentricity: 0.08 (reduced)
    omega=2.85,                          
    inc=(5*u.deg).to(u.rad).value,      # Inclination: 5 degrees (reduced)
    Omega=4.05                           
)

# Add Planet 5 (outer) - NO MOON
sim.add(
    m=(0.3*u.Mjup).to(u.Msun).value,    # Mass: 0.3 Jupiter masses (~95 M⊕)
    a=4.8,                               # Semi-major axis: 4.8 AU (moved inward)
    e=0.25,                              # Eccentricity: 0.25 (reduced)
    omega=5.86,                          
    inc=(15*u.deg).to(u.rad).value,     # Inclination: 15 degrees (reduced)
    Omega=5.72                           
)

# Add Planet 6 (new outer planet for stability buffer)
sim.add(
    m=(0.5*u.Mjup).to(u.Msun).value,    # Mass: 0.5 Jupiter masses
    a=8.5,                               # Semi-major axis: 8.5 AU
    e=0.15,                              # Eccentricity: 0.15
    omega=1.23,                          
    inc=(10*u.deg).to(u.rad).value,     # Inclination: 10 degrees
    Omega=3.45                           
)
...
# Conservative placement at 3% of Hill radius
planet2_hill = hill_radius(0.7, (1.2*u.Mearth).to(u.Msun).value, 0.867)
moon1_distance = 0.03 * planet2_hill  # 3% of Hill radius
sim.add(
    primary=sim.particles[2],            # Orbits around Planet 2 (1.2 M⊕)
    m=(7.35e22*u.kg).to(u.Msun).value,  # Mass: ~Luna mass
    a=moon1_distance,                    # Safe distance at 3% Hill radius
    e=0.0005,                            # Very low eccentricity
    omega=2.35,                          
    inc=(0.05*u.deg).to(u.rad).value,   # Very low inclination
    Omega=4.87 
)
print(f"Planet 2 (1.2 M⊕) Hill radius: {planet2_hill*1.496e8:.0f} km")
print(f"  Moon 1 distance: {moon1_distance*1.496e8:.0f} km ({100*moon1_distance/planet2_hill:.1f}% Hill)")

# Add Moon 2 orbiting Planet 3 (index 3 in particles list)
planet3_hill = hill_radius(1.3, (4*u.Mearth).to(u.Msun).value, 0.867)
moon2_distance = 0.025 * planet3_hill  # 2.5% of Hill radius
sim.add(
    primary=sim.particles[3],            # Orbits around Planet 3 (4 M⊕)
    m=(1.8e23*u.kg).to(u.Msun).value,   # Mass: ~2.5x Luna mass
...
print(f"Planet 3 (4 M⊕) Hill radius: {planet3_hill*1.496e8:.0f} km")
print(f"  Moon 2 distance: {moon2_distance*1.496e8:.0f} km ({100*moon2_distance/planet3_hill:.1f}% Hill)")

# Add Moon 3 orbiting Planet 4 (index 4 in particles list) - Inner moon
planet4_hill = hill_radius(2.1, (6.5*u.Mearth).to(u.Msun).value, 0.867)
moon3_distance = 0.02 * planet4_hill  # 2% of Hill radius (very conservative)
sim.add(
    primary=sim.particles[4],            # Orbits around Planet 4 (6.5 M⊕)
    m=(2e23*u.kg).to(u.Msun).value,     # Mass: ~2.7x Luna mass
    a=moon3_distance,                    # Safe distance
    e=0.0005,                            # Very low eccentricity
    omega=1.47,                          
    inc=(0.03*u.deg).to(u.rad).value,   # Very low inclination
    Omega=3.91 
)
print(f"Planet 4 (6.5 M⊕) Hill radius: {planet4_hill*1.496e8:.0f} km")
print(f"  Moon 3 distance: {moon3_distance*1.496e8:.0f} km ({100*moon3_distance/planet4_hill:.1f}% Hill)")

# Add Moon 4 orbiting Planet 4 - Outer moon (well-separated from Moon 3)
moon4_distance = 0.05 * planet4_hill  # 5% of Hill radius (2.5x Moon 3's distance)
sim.add(
    primary=sim.particles[4],            # Orbits around Planet 4 (6.5 M⊕)
    m=(1.2e23*u.kg).to(u.Msun).value,   # Mass: ~1.6x Luna mass
    a=moon4_distance,                    # Safe distance, 2.5x Moon 3's distance
    e=0.0005,                            # Very low eccentricity
    omega=4.71,                          
    inc=(0.04*u.deg).to(u.rad).value,   # Very low inclination
    Omega=2.19 
)
print(f"  Moon 4 distance: {moon4_distance*1.496e8:.0f} km ({100*moon4_distance/planet4_hill:.1f}% Hill)")
print(f"  Moon separation ratio: {moon4_distance/moon3_distance:.1f}:1")

# Move to center of mass frame (important for stability)
sim.move_to_com()

# Use IAS15 for moon systems - accurate for hierarchical systems
sim.integrator = "ias15"
sim.dt = 0.0005  # Small timestep for accuracy

print("\n⚠️  Using IAS15 integrator for moon stability (slower but accurate)")
print("    Expect ~40-90 seconds for 10,000 year simulation\n")
...
colors_planets = ['#FF6B6B', '#4ECDC4', '#6BCB77', '#45B7D1', '#FFA07A', '#FFB6C1']
for i, (planet_idx, color) in enumerate(zip([1, 2, 3, 4, 5, 6], colors_planets)):
...
ax1.set_title('6-Planet System Configuration (Optimized for Moon Stability)', fontsize=14, fontweight='bold')
...
moon1_orb = sim.particles[7].orbit(primary=sim.particles[2])
theta = np.linspace(0, 2*np.pi, 200)
...
moon_rel_x = (sim.particles[7].x - sim.particles[2].x) * 1.496e8
moon_rel_y = (sim.particles[7].y - sim.particles[2].y) * 1.496e8
ax2.plot(moon_rel_x, moon_rel_y, 'o', color='#95E1D3', markersize=6)
ax2.set_xlabel('x [km]', fontsize=10)
ax2.set_ylabel('y [km]', fontsize=10)
ax2.set_title('Moon 1 → Planet 2 (1.2 M⊕)', fontsize=11, fontweight='bold')
ax2.set_aspect('equal')
ax2.grid(True, alpha=0.3)

# Plot 3: Moon 2 around Planet 3
ax3 = plt.subplot(2, 3, 5)
moon2_orb = sim.particles[8].orbit(primary=sim.particles[3])
...
moon_rel_x = (sim.particles[8].x - sim.particles[3].x) * 1.496e8
moon_rel_y = (sim.particles[8].y - sim.particles[3].y) * 1.496e8
ax3.plot(moon_rel_x, moon_rel_y, 'o', color='#FFD93D', markersize=6)
ax3.set_xlabel('x [km]', fontsize=10)
ax3.set_ylabel('y [km]', fontsize=10)
ax3.set_title('Moon 2 → Planet 3 (4 M⊕)', fontsize=11, fontweight='bold')
ax3.set_aspect('equal')
ax3.grid(True, alpha=0.3)

# Plot 4: Moons 3 & 4 around Planet 4
ax4 = plt.subplot(2, 3, 6)
moon3_orb = sim.particles[9].orbit(primary=sim.particles[4])
moon4_orb = sim.particles[10].orbit(primary=sim.particles[4])
...
moon_rel_x = (sim.particles[10].x - sim.particles[4].x) * 1.496e8
moon_rel_y = (sim.particles[10].y - sim.particles[4].y) * 1.496e8
ax4.plot(moon_rel_x, moon_rel_y, 'o', color='#A8DADC', markersize=6)

ax4.set_xlabel('x [km]', fontsize=10)
ax4.set_ylabel('y [km]', fontsize=10)
ax4.set_title('Moons 3 & 4 → Planet 4 (6.5 M⊕)', fontsize=11, fontweight='bold')
...
print(f"\nSimulating {simulation_time} years with {n_steps} snapshots...")
print(f"Initial energy: {E0:.6e}")
print(f"Total bodies: {sim.N} (1 star + 6 planets + 4 moons)")
...
planet_labels = ['Planet 1 (0.6 M⊕, 0.5 AU)', 
                 'Planet 2 (1.2 M⊕, 0.7 AU)',
                 'Planet 3 (4 M⊕, 1.3 AU)',
                 'Planet 4 (6.5 M⊕, 2.1 AU)', 
                 'Planet 5 (95 M⊕, 4.8 AU)',
                 'Planet 6 (159 M⊕, 8.5 AU)',
                 'Moon 1 → P2',
                 'Moon 2 → P3',
                 'Moon 3 → P4 (inner)',
                 'Moon 4 → P4 (outer)']
colors = ['#FF6B6B', '#4ECDC4', '#6BCB77', '#45B7D1', '#FFA07A', '#FFB6C1',
          '#95E1D3', '#FFD93D', '#C77DFF', '#A8DADC']

# Plot planets with solid lines
for j in range(6):  # First 6 are planets
    ax.plot(times, smas[j], label=planet_labels[j], color=colors[j], linewidth=2)

# Plot moons with dashed lines
for j in range(6, n_bodies):  # Remaining are moons
...
ax.set_title('Orbital Evolution: Semi-major Axes (Stabilized System)', fontsize=14, fontweight='bold')
...
ax.set_title('Orbital Evolution: Eccentricities (Stabilized System)', fontsize=14, fontweight='bold')
ax.legend(loc='upper right', fontsize=9, ncol=2)
ax.grid(True, alpha=0.3)
ax.set_xlim(0, simulation_time)
ax.set_ylim(0, 0.35)
...
# Summary statistics
print("\n" + "="*70)
print("ORBITAL STABILITY SUMMARY - OPTIMIZED SYSTEM")
print("="*70)
for j in range(n_bodies):
...
print("="*70)
print("\nKey Improvements:")
print("• Increased planet masses for stronger Hill spheres")
print("• Reduced moon orbital distances (2-5% of Hill radius)")
print("• Lowered moon eccentricities to near-circular orbits")
print("• Added Planet 6 as outer stability buffer")
print("• Moved Planet 5 inward to reduce perturbations")
print("• Better moon-moon separation for Planet 4's system")
print("="*70)
